...
function [out,medie_left,medie_right]=nbt_computeSED_NOnorm(topog,chanlocs,n,eyechannels)
...
index1=zeros(1,length(chanlocs)); %indexes of LE electrodes

if(~isempty(eyechannels))
    indexl(1,1) = eyechannels(1);
end
...
for k=1:length(chanlocs)
 ...
 % Find electrodes in Right Eye area (RE)
    ...
    index2=zeros(1,length(chanlocs)); %indexes of RE electrodes
    
    if(~isempty(eyechannels))
    indexl(1,2) = eyechannels(2);
    end
    
    for g=1:length(chanlocs)
        try
   ...
   % Find electrodes in Posterior Area (PA)
    dimback=0;
    index3=zeros(1,length(chanlocs));
    for h=1:length(chanlocs)
        try
