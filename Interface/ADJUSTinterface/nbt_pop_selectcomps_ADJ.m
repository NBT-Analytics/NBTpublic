...
for ri = compnum
	...
	if isempty( button )
		...
		command = sprintf('nbt_pop_prop_ADJ( %s, 0, %d, %3.15f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f ,%f,%f,%f);', inputname(1), ri, button,...
            ~isempty(intersect(horiz,ri)), ~isempty(intersect(vert,ri)), ~isempty(intersect(blink,ri)), ~isempty(intersect(disc,ri)), soglia_DV, diff_var(ri), soglia_K, meanK(ri),...
        soglia_SED, SED(ri),soglia_SAD, SAD(ri), soglia_V, maxvar(ri));
