% nbt_set_name(Signal,SignalInfo)
%
%
% Usage:
%   nbt_set_name(Signal,SignalInfo)
...
%------------------------------------------------------------------------------------
% Originally created by Rick Jansen (2011), see NBT website for current email address
%------------------------------------------------------------------------------------
%
...
function nbt_set_name(Signal,SignalInfo)
hh = findobj('Tag','NBT');

if isempty(Signal)
    mm = uicontrol('parent', hh, 'style','text','string','No Signal loaded','position',[0   10   800   70]);
else
    mm = uicontrol('parent', hh, 'style','text','string',[{['Current Signal: ' SignalInfo.SignalName '@'  SignalInfo.file_name]},{['# channels: ' ...
